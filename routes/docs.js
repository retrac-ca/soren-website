  ...
  res.render("docs", {
    ...
    commandGroups: [
      ...
      },
      {
        ...
        icon: "📅",
        commands: [
          { name: "/newevent",          who: "Event Creator", description: "Create an event in a single command. Required: channel, title, start. Optional: end, description, timezone (autocomplete), recurrence (autocomplete), recur_interval (days between occurrences for Custom recurrence), role, role2, role3 (@mention pickers — role2 and role3 require Premium), reminder (minutes before event), max_rsvp (accepted RSVP cap), cutoff (date/time when RSVPs close). A public discussion thread is created automatically on the event embed." },
          { name: "/duplicateevent",    who: "Event Creator", description: "Clone an existing event to a new date and time. All settings carry over — provide the event ID and new start time. End time is optional." },
          ...
          { name: "/exportevents",      who: "Everyone",      description: "Export all upcoming events in this server as an .ics file, compatible with Google Calendar, Apple Calendar, and Outlook." },
